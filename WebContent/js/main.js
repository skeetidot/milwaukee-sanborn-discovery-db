...
var mapOptions = {
	...
	minZoom : 11,
	maxZoom : 21,
	maxBounds : L.latLngBounds([ 42.84, -87.82 ], [ 43.19, -88.07 ]), // panning
	// bounds
	// so
	// the
	// user
	...
	// from
	// Milwaukee
	bounceAtZoomLimits : false, // Set it to false if you don't want the map to
	// zoom beyond min/max zoom and then bounce back
	// when pinch-zooming
	layers : [ Esri_WorldGrayCanvas, sanborn ], // Set the layers to build into
	// the layer control
}
...
//		});
		...
		$.ajax({
			...
			// If the request succeeds, the record is submitted
			// Display an alert popup with a confirmation message
			success : function(records) {
				alert("The record was submitted successfully");
			},
		...
		// Make an Ajax request to query all records in the database using POST
		$.ajax({
			url : 'SanbornServlet',
			type : 'POST',
			data : {
			...
			// If the request succeeds, call the showAllRecords() function
			// to redraw the map with all the records
			success : function(records) {
		...
		// to update the map extent to the record that was just submitted
		$(document).ajaxComplete(function(records) {
			//onPlaceChanged();
		});
	...
	// FUNCTION THAT HAS JUST ENDED*/
	// });

	/** ***************************************************************************** */
	/*
	 * JAVASCRIPT RELATED TO UPDATING THE HISTORIC MAPS WHEN OPACITY SLIDER IS
	 * INITIATED
	 */
